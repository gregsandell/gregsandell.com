...
loadingOpacity : 0.75,
showCredits : false, // you can set this to false if you want  (gregsandell.com:  done!)
...
keyHandler : function(e) {
	...
	if (op != null) {
		...
		try { if (!hs.expanders[hs.focusKey].enableKeyListener) return true; } catch (e) {}
...
getExpander : function (el) {
	try {
		...
		if (typeof(el) == 'string') {
			id = el;
			el = hs.$(id);
			if (el) key = hs.getWrapperKey(el);
			if (key == null || key == -1) {				
				for (i = 0; i < hs.expanders.length; i++) {
					exp = hs.expanders[i];
					if (exp && (exp.thumb.id == id || exp.a.id == id)) {
						return exp;
					}
				}
			}
